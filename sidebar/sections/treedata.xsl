...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <xsl:template name="sidebar-treedata">
  ...
  <xsl:template name="package-content">
    ...
    <xsl:template match="package">
        ...
        or (count(/project/file/constant[contains(@package, $full_name)]) > 0))
        ">
            '{'+
                '"id": <xsl:if test="@name=''">"pkg-\"</xsl:if><xsl:if test="not(@name='')">"pkg-<xsl:value-of select="@name" />"</xsl:if>,'+
                '"text": <xsl:if test="@name=''">"\"</xsl:if><xsl:if test="not(@name='')">"<xsl:value-of select="@name" />"</xsl:if>,'+
                '"iconCls":"icon-pkg",'+
                '"cls":"package",'+
                ...
                '"children":['+
                    ...
                    <xsl:for-each select="/project/file/constant[@package=$full_name]">
                    <xsl:sort select="name" />
                        '{'+
                            '"href":"<xsl:value-of select="$root" />files/<xsl:value-of select="../@generated-path" />",'+
                            '"text":"<xsl:value-of select="name" />",'+
                            '"id":"<xsl:value-of select="name" />",'+
                            '"filename":"<xsl:value-of select="@path" />",'+
                            '"isClass":true,'+
                    ...
                    <xsl:sort select="name" />
                        '{'+
                            '"href":"<xsl:value-of select="$root" />files/<xsl:value-of select="../@generated-path" />#method-anchor-<xsl:value-of select="name" />",'+
                            '"text":"<xsl:value-of select="name" />",'+
                            '"id":"<xsl:value-of select="name" />",'+
                            '"filename":"<xsl:value-of select="@path" />",'+
                            '"isClass":true,'+
